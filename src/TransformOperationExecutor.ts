...
export class TransformOperationExecutor {
  ...
  transform(
    ...
    subValue[targetType.options.discriminator.property] = (targetType as { options: TypeOptions }).options.discriminator.subTypes.find(subType=> { return subType.name === subValue[targetType.options.discriminator.property]; }).name;
}else{
